...
public class GestionTareasController extends Controller {
	 ...
	 public Result grabaTareaModificada(Long idTarea) {
		...
		if(tarea.getTablero() != null) {
			String columnaId = requestData.get("columnaId");
			Long idColumna = Long.valueOf(columnaId);
			if(idColumna == 0) {
				tareaService.quitarColumna(idTarea);
			}
			else {
				columnaService.addTareaColumna(idColumna, idTarea);
			}
		}
